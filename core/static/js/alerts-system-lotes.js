class AlertsSystemLotes {
                                ...
                                <div class="col-md-4">
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="mb-3">
                                                <label class="form-label"><strong>Geofence:</strong></label>
                                                <input type="text" class="form-control" id="geofenceNome" readonly>
                                            </div>
                                            <div class="mb-3">
    ...
    createAlertElement(alert) {
        ...
        const geofence = alert.data.geofence ? `📍 ${alert.data.geofence}` : '';
        ...
        element.innerHTML = `
            <div style="display: flex; align-items: center; justify-content: space-between;">
                <div style="flex: 1;">
                    ...
                    ${geofence ? `<div style="font-size: 12px; margin-bottom: 5px; opacity: 0.9;">${geofence}</div>` : ''}
    ...
    abrirModalTratamento(alert) {
        ...
        document.getElementById('geofenceNome').value = alert.data.geofence || 'Fora de geofence';
        ...
        this.modal._element.addEventListener('shown.bs.modal', () => {
            this.carregarMapa(alert.data.lat, alert.data.lng);
        }, { once: true });
    }
        ...
        try {
            const response = await fetch('/tratar-evento/', {
                ...
                body: JSON.stringify({
                    ...
                    geofence: this.alertAtual.data.geofence,
    ...
    async checkAlerts() {
        try {
            console.log('🔍 Verificando alertas em lotes...');
            const response = await fetch('/verificar-alertas-lotes/');
            const data = await response.json();
            
            console.log('📡 Resposta da API:', data);
            
            if (data.eventos && Array.isArray(data.eventos)) {
                console.log(`📋 Encontrados ${data.eventos.length} eventos de ${data.total_equipamentos} equipamentos`);
    ...
    startChecking() {
        // Verificar a cada 30 segundos (mais tempo devido ao processamento em lotes)
        setInterval(() => {
            this.checkAlerts();
        }, 30000);
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Inicializando sistema de alertas em lotes...');
    
    // Verificar se já existe um sistema de alertas
    if (window.alertsSystemLotes) {
        console.log('⚠️ Sistema de alertas em lotes já existe, não inicializando novamente');
        return;
    }
    
    window.alertsSystemLotes = new AlertsSystemLotes();
    console.log('✅ Sistema de alertas em lotes inicializado!');
});

// Funções globais
window.tratarEventoLotes = (alertId) => {
    if (window.alertsSystemLotes) {
        const alert = window.alertsSystemLotes.alerts.find(a => a.id === alertId);
        if (alert) {
            window.alertsSystemLotes.abrirModalTratamento(alert);
        }
    }
};

window.removeAlertLotes = (alertId) => {
    if (window.alertsSystemLotes) {
        window.alertsSystemLotes.removeAlert(alertId);
    }
}; 